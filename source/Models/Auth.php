...
class Auth extends Model
{
    ...
    public static function user(): ?User
    {
        ...
        return (new User())->findById($session->authUser->userID);
    }

    /**
     * @return null|AppClient
     */
    public static function client(): ?AppClient
    {
        $session = new Session();
        if (!$session->has("authUser")) {
            return null;
        }
        return (new AppClient())->findById($session->authUser->clientID);
    }
    ...
    public function login(string $email, string $password, bool $save = false, int $level = 1): bool
    {
        ...
        //LOGIN
        (new Session())->set("authUser", ['userID' => $user->id,'clientID' => $user->client_id]);
        return true;
    }
