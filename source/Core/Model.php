...
/**
 * RECPECAO | Class Model Layer Supertype Pattern
 *
 * @author Edson Costa <edson@onlogexpress.com.br>
...
abstract class Model
{
    ...
    /** @var string $entity join database table */
    protected $entity_join;

    /** @var string $fiel join database table */
    protected $join_fieds_keys;
    ...
    public function __construct(
        ...
        array $protected,
        array $required,
        array $join_fieds_keys,
        array $entity_join,
        ?string $columns = "*"
    ) {
        $this->entity = $entity;
        $this->protected = array_merge($protected, ['data_cadastro']);
        $this->required = $required;
        $this->entity_join = $entity_join;
        $this->join_fieds_keys = $join_fieds_keys;
    ...
    {
        $this->columns =  ($this->join != null) ? $this->columns : $columns;
        if ($terms) {
            $this->query = "SELECT {$this->columns} FROM {$this->entity} {$this->join} WHERE {$terms}";
            parse_str($params, $this->params);
            return $this;
        }

        $this->query = "SELECT {$this->columns} FROM {$this->entity} {$this->join}";
        return $this;
    }


    public function join(?string $term = "INNER JOIN")
    {
        $join = null;
        foreach ($this->entity_join as $key => $jfield) {
            $join .= " {$term} {$key} ON {$key}.{$jfield} = {$this->entity}.{$this->join_fieds_keys[$key]}";
        }
    ...
    public function findQuerySQL(?string $stmt = null)
    {
        $this->query = $stmt;
        return $this;
    }

    public function findByLike(?string $terms = null, string $columns = "*")
    {
        if ($terms) {
